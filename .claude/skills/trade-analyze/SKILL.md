...
**Step 4 — Verify Output (CRITICAL)**

You MUST run the backtest verification script after generating the report to catch any logical errors:

```bash
python scripts/backtest_analysis.py TRADE-ANALYSIS-<TICKER>.md
```

If the script returns a **LOGIC ERROR** (e.g., Stop Loss on wrong side), you MUST correct the markdown file before presenting it to the user.

**Step 5 — Confirm and Present**

After verification and any necessary corrections:
- Show the Trade Score, Grade, and Signal
- Display the verification pass/fail status
